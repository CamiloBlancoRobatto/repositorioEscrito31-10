...
namespace capaLogica
{
    public class CL_Venta
    {
        ...
        private void GenerarPDF(int precioTotal, string tipoPropiedad, int iva, int descuento, int precio)
        {
            ...
            string rutaPDF = System.IO.Path.Combine(rutaEscritorio, "FACTURA_PROPIEDAD.pdf");

            using (var doc = new iTextSharp.text.Document())
            {
                iTextSharp.text.pdf.PdfWriter.GetInstance(doc, new System.IO.FileStream(rutaPDF, System.IO.FileMode.Create));
                doc.Open();
                doc.Add(new iTextSharp.text.Paragraph("Detalles de la FACTURA"));
                doc.Add(new iTextSharp.text.Paragraph($"Tipo de Propiedad: {tipoPropiedad}"));
                doc.Add(new iTextSharp.text.Paragraph($"IVA aplicado: {iva}%"));
                doc.Add(new iTextSharp.text.Paragraph($"Precio: {precio}"));
                doc.Add(new iTextSharp.text.Paragraph($"Descuento si es terreno: {descuento}"));
                doc.Add(new iTextSharp.text.Paragraph($"Total a pagar: {precioTotal}"));
                doc.Close();
            }
        }
